(function($, undefined) {
    ...
    var $activeElm, preventShow, extend = $.extend,
    ms = $.mobiscroll,
    ...
    ms.classes.Widget = function(el, settings, inherit) {
        var $ariaDiv, $ctx, $header, $markup, $overlay, $persp, $popup, $wnd, $wrapper, buttons, btn, doAnim, hasButtons, isModal, lang, modalWidth, modalHeight, posEvents, preset, preventPos, s, scrollLock, setReadOnly, theme, wasReadOnly, wndWidth, wndHeight, that = this,
        $elm = $(el),
        elmList = [],
        posDebounce = {};
        ...
        function event(name, args) {
            var ret;
            args.push(that);
            $.each([userdef, theme, preset, settings],
            function(i, v) {
                if (v && v[name]) {
        ...
        that.attachShow = function($elm, beforeShow) {
            ...
            if (s.display !== "inline") {
                ...
                }).on((s.showOnFocus ? "focus.dw": "") + (s.showOnTap ? " click.dw": ""),
                function(ev) {
        ...
        that.show = function(prevAnim, prevFocus) {
            ...
            if (isModal) {
                ...
                if (s.scrollLock) {
                    $markup.on("touchstart touchmove",
                    function(ev) {
                        if (scrollLock) {
                            ev.preventDefault()
                        }
                ...
                if (has3d && doAnim && !prevAnim) {
                    $markup.addClass("dw-in dw-trans").on(animEnd,
                    function() {
            ...
            $markup.on("selectstart mousedown", prevdef).on("click", ".dwb-e", prevdef).on("keydown", ".dwb-e",
            function(ev) {
                if (ev.keyCode == 32) {
                    ev.preventDefault();
                    ev.stopPropagation();
        ...
        that.hide = function(prevAnim, btn, force) {
            ...
            if ($markup) {
                ...
                if (has3d && isModal && doAnim && !prevAnim && !$markup.hasClass("dw-trans")) {
                    $markup.addClass("dw-out dw-trans").find(".dw").addClass("dw-" + doAnim).on(animEnd,
                    function() {
                        onHide(prevAnim)
                    })
                } else {
    ...
    $(document).on("mouseover mouseup mousedown click",
    function(ev) {
        if (ms.tapped) {
            ev.stopPropagation();
            ev.preventDefault();
    ...
    classes.Scroller = function(el, settings, inherit) {
        var $markup, btn, isScrollable, itemHeight, s, trigger, valueText, click, moved, start, startTime, stop, p, min, max, target, index, lines, timer, that = this,
        $elm = $(el),
        iv = {},
        pos = {},
        pixels = {},
    ...
    $.mobiscroll.presets.scroller.select = function(inst) {
        var change, grIdx, gr, group, input, optIdx, option, prev, prevent, timer, w, orig = $.extend({},
        inst.settings),
        ...
        return {
            ...
            onMarkupReady: function(dw) {
                ...
                $(".dwwl" + grIdx, dw).on("mousedown touchstart",
                function() {
                    clearTimeout(timer)
                });
                if (groupHdr) {
    ...
    preset = function(inst) {
        ...
        var i, k, keys, values, wg, start, end, hasTime, mins, maxs, orig = $.extend({},
        inst.settings),
        ...
        if (p.match(/date/i)) {
            ...
            for (k = 0; k < 3; k++) {
                if (k == o.y) {
                    ...
                    for (i = start; i <= end; i++) {
                        keys.push(i);
                        values.push((dord.match(/yy/i) ? i: (i + "").substr(2, 2))+"年" + (s.yearSuffix || ""))
                    }
                    ...
                    if (k == o.m) {
                        ...
                        keys = [];
                        for (i = 0; i < 12; i++) {
                            var str = dord.replace(/[dy]/gi, "").replace(/mm/, (i < 9 ? "0" + (i + 1) : i + 1)+"月" + (s.monthSuffix || "")).replace(/m/, i + 1 + (s.monthSuffix || ""));
                            keys.push(i);
                        ...
                        if (k == o.d) {
                            ...
                            for (i = 1; i < 32; i++) {
                                keys.push(i);
                                values.push((dord.match(/dd/i) && i < 10 ? "0" + i: i) +"日"+ (s.daySuffix || ""))
                            }
